"""Scrape the official English Empack Madrid 2026 exhibitor directory."""
...
EVENT_NAME = "Empack Madrid 2026"
EVENT_YEAR = "2026"
SOURCE_URL = "https://www.empackmadrid.com/en/exhibitors/"
BASE_URL = "https://www.empackmadrid.com"
ROOT = Path(__file__).resolve().parent
OUTPUT = ROOT / "output"
CSV_PATH = OUTPUT / "EMPACK_MADRID_2026_exhibitors.csv"
JSON_PATH = OUTPUT / "EMPACK_MADRID_2026_exhibitors.json"
...
BLOCKED = {
    "empackmadrid.com", "linkedin.com", "facebook.com", "instagram.com",
...
def country_from_text(text: str) -> str:
    countries = {
        "spain": "Spain", "united kingdom": "United Kingdom", "england": "United Kingdom",
        "germany": "Germany", "france": "France", "italy": "Italy",
        "netherlands": "Netherlands", "belgium": "Belgium", "turkey": "Türkiye",
        "portugal": "Portugal", "poland": "Poland", "china": "China",
        "finland": "Finland", "sweden": "Sweden", "algeria": "Algeria",
        "egypt": "Egypt", "latvia": "Latvia", "vietnam": "Viet Nam",
    }
    lower = text.casefold()
...
def list_exhibitors(session: requests.Session) -> list[dict[str, str]]:
    ...
    for page in range(1, 14):
        ...
        for card in soup.select("article.card"):
            ...
            stand_node = card.select_one(".stand-header__stand-number")
            stand = clean(stand_node.get_text(" ", strip=True)) if stand_node else ""
            description = card.select_one(".hit__description-wrapper")
            record = base_record(clean(heading.get_text(" ", strip=True)), stand, profile_url)
            record["desc"] = clean(description.get_text(" ", strip=True)) if description else ""
            records.append(record)
    if len(records) != 184:
        raise RuntimeError(f"Expected 184 2026 exhibitors, found {len(records)}")
    return records
...
def parse_location(body: str, record: dict[str, str]) -> None:
    ...
    match = re.search(
        r"\b\d{4,5}\s+([A-Z][^,\n]+?)(?:,\s*|\s+)(Spain|United Kingdom|Germany|France|Italy|Netherlands|Belgium|Turkey|Portugal|Poland|China)\b",
        body,
        re.I,
    )
    if match:
...
def parse_profile(record: dict[str, str], content: str) -> dict[str, str]:
    ...
    if not record["desc"]:
        about = re.search(r"About us\s+(.*?)(?:Products & services|News|Contact Info|$)", body, re.I)
    ...
    record["domain"] = next(
        (
            ...
            and ("website" in clean(a.get_text(" ", strip=True)).casefold()
