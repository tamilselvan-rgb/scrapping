...
EVENT = "77TH_INTERNATIONAL_ASTRONAUTICAL_CONGRESS_IAC_ANTALYA_2026"
ROOT = Path(__file__).resolve().parent
OUT = ROOT / "output"
LIST_URL = "https://www.iac2026.org/iac-2026-exhibitors"
...
BLOCKED = {
    "iac2026.org", "facebook.com", "instagram.com", "twitter.com", "x.com",
...
def clean(value):
    value = html.unescape(str(value or "")).replace("Servi�os", "Serviços")
...
def listing_records():
    ...
    for row in table.select("tr")[1:]:
        ...
        if len(cells) != 3:
            continue
        organization, co_exhibitor, booth = cells
        if booth:
            current_booth = booth
        company_name = organization or co_exhibitor
        if not company_name:
            continue
        records.append({
            "company_name": company_name,
            "booth": booth or current_booth,
            "description": "",
            "email": "",
            "mobile_primary": "",
            "domain": "",
...
def enrich_domain(record):
    ...
    try:
        response = requests.post(
            ...
            json={"q": f'"{record["company_name"]}" official website 2026 IAC Antalya',
                  "gl": "tr", "hl": "en", "num": 10},
            timeout=45,
        )
        response.raise_for_status()
...
def main():
    ...
    print(f"2026 listing exhibitors and co-exhibitors: {len(records)}")
    ...
    print("=== Verification Report: IAC ANTALYA 2026 ===")
